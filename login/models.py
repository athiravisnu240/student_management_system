...
class Attendance(models.Model):
    ...
    def __str__(self):
        return f"{self.stud_id} ({self.date})"


class Slot(models.Model):
    period_id = models.IntegerField(
        validators=[MinValueValidator(1), MaxValueValidator(8)], primary_key=True
    )
...
class Leave(models.Model):
    ...
    leave_type = models.CharField(
        max_length=9, choices=LEAVE_CHOICE, default=None, blank=False
    )
