    ...
    if request.method == "POST":
        ...
        if faco.exists():
            if faco.get().f_password == p:
                ...
                initial(u, d)
                return redirect(reverse_lazy("updatedindex", kwargs={"pk": u}))
            else:
...
def updatedprofile(request, *args, **kwargs):
    fac = kwargs.get("pk")
    if request.method == "POST":
        try:
    ...
    faco = Faculty.objects.filter(fac_id=fac).first()
    ...
    return render(
        request,
        "updatedprofile.html",
        {"clas": clas, "fac": faco, "dept": dept},
    )


def editatt(request, *args, **kwargs):
    fac = kwargs.get("pk")
...
def updatedindex(request, *args, **kwargs):
    fac = kwargs.get("pk")
    dept = Department.objects.filter(dept_id=dep)
    faco = Faculty.objects.filter(fac_id=fac).first()
    teach = Teache.objects.filter(fac_id=fac)
    ...
    for i in teach:
        clas.append([i.class_id.class_id, i.course_id.course_id])
    return render(request, "updatedindex.html", {"clas": clas, "teach": teach, "fac":faco})


def updatedadd(request, *args, **kwargs):
    fac = kwargs.get("pk")
    dept = Department.objects.filter(dept_id=dep)
    faco = Faculty.objects.filter(fac_id=fac)
    teach = Teache.objects.filter(fac_id=fac)
...
def fac_report(request, *args, **kwargs):
    fac = kwargs.get("pk")
    if request.method == "POST":
        dict = request.POST
        for i in dict.keys():
    ...
    atte = (
        ...
        .filter(course_id=cou, fac_id=fac)
        .order_by("stud_id", "date")
    )
